  ...
  endpoints: (builder) => ({
    getStores: builder.query<Store[], void>({
      async queryFn(_arg, _api, _extraOptions, baseQuery) {
        if (await isOnline()) {
          const result = await baseQuery("/stores");
    ...
    createStore: builder.mutation<Store, CreateStorePayload>({
      async queryFn(body, _api, _extraOptions, baseQuery) {
        if (await isOnline()) {
          const result = await baseQuery({ url: "/stores", method: "POST", body });
    ...
    updateStore: builder.mutation<Store, { id: string; data: Partial<CreateStorePayload> }>({
      async queryFn({ id, data }, _api, _extraOptions, baseQuery) {
        if (await isOnline()) {
          const result = await baseQuery({ url: `/stores/${id}`, method: "PUT", body: data });
          if (!result.error) return { data: result.data as Store };
          if (typeof result.error.status === "number" && result.error.status < 500) return { error: result.error };
        }

        await updateOfflineEntity("stores", id, data);
        return { data: (await getLocalStores()).find((store) => store.id === id) as Store };
      },
      invalidatesTags: ["Stores" as any],
    }),
    ...
    deleteStore: builder.mutation<void, string>({
      async queryFn(id, _api, _extraOptions, baseQuery) {
        if (await isOnline()) {
          const result = await baseQuery({ url: `/stores/${id}`, method: "DELETE" });
